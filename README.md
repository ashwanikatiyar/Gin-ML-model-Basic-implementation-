...
The predicted output is calculated using the following sigmoid activation function:

$$ \hat{y} = \sigma(w \cdot x + b) $$

**Where:**
*   **$x$**: The input [text feature vector](https://developers.google.com) (e.g., TF-IDF or Word Embeddings).
*   **$w$**: The [learned weights](https://pytorch.org) that determine the importance of each feature.
*   **$b$**: The [bias term](https://machinelearningmastery.com) (intercept).
*   **$\sigma$**: The [Sigmoid function](https://en.wikipedia.org), which maps any real-valued number into a probability between 0 and 1.
...
📌 Note: Training updates the in-memory model.
...
Model is in-memory only (lost on server restart)
No persistence or model versioning
Simple Bag-of-Words (no embeddings)
